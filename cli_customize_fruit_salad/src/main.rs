...
fn main() {
    ...
    display_fruit_salad(fruit_salad.clone());

    // save the resulting salad to new csv
    let result_salad = fruit_salad.join(",");
    std::fs::write("result.csv", result_salad).expect("Could not write file");
}
